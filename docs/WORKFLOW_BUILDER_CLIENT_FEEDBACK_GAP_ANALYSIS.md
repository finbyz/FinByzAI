...
| **E07** | [Workflow canvas](../workflow/src/components/WorkflowCanvas.tsx#L105) | Provides compact visible `+` insertion points, HubSpot-style action menus and contextual **Paste below** targets, color-separated branch paths, automatic lane layout, atomic rewiring, structural action relocation, and per-output derived END markers. |
...
| **E09** | [Editor state and commands](../workflow/src/state/WorkflowContext.tsx#L421) | Implements save, autosave, simulation, atomic insert/relocate, immutable action/following-action copy snapshots, dependency-safe one-shot paste, undo/redo, and conflict recovery. |
...
| UX-06 | Copy and paste actions or branch sections | **Implemented** | Each action menu offers **Copy action** and **Copy all actions from here**. Copy mode exposes **Paste below** beside valid insertion points, remaps internal IDs/references, rejects unsafe earlier-output and Go To destinations, restricts branches/terminating sections to END points, and stops at shared convergence. E07, E09 | Copy is workflow-local, one-shot, cancellable, and recorded as one undoable command. | P0 |
...
- Canvas insertion, structural action relocation, and HubSpot-style action/following-action paste are atomic graph commands so undo/redo, autosave, conflict detection, and validation remain reliable.

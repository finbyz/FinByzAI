...
import { availableOutputNodes, conditionToFilterGroups, emptyPredicate, filterGroupsToCondition, isRequiredAuthoringValueMissing, outputCatalog, parseCondition, parseWebhookPayload } from '../lib/inspectorAuthoring'
...
function DurationEditor({ seconds, onChange }: { seconds: number; onChange(seconds: number, duration: number, unit: string): void }) {
	...
	const updateDuration = (nextDuration: number, nextUnit = unit) => {
		const nextMultiplier = durationUnits.find((item) => item.value === nextUnit)?.multiplier || 1
		onChange(Math.round(nextDuration * nextMultiplier), nextDuration, nextUnit)
	}
	return <div className="grid grid-cols-[minmax(0,1fr)_130px] gap-2"><div><label className={labelClass}>Duration</label><input type="number" min={unit === 'seconds' ? 60 : 1} step={1} className={inputClass} value={Number.isFinite(duration) ? duration : 1} onChange={(event) => updateDuration(Number(event.target.value))} /></div><div><label className={labelClass}>Unit</label><select className={inputClass} value={unit} onChange={(event) => { const nextUnit = event.target.value; setUnit(nextUnit as typeof unit); updateDuration(duration, nextUnit) }}>{durationUnits.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select></div></div>
}
...
export function SendEmailEditor({
	...
	return <>
		...
		<InspectorSection title="Recipient and sending details" description="Choose who receives the message and optionally override workflow-level sender defaults.">
			{recipientEditor}
			<div className="grid grid-cols-2 gap-2"><div><label className={labelClass}>From name <span className="font-normal">(optional)</span></label><input className={inputClass} value={String(config.sender_name || '')} onChange={(event) => update({ ...config, sender_name: event.target.value }, 'sender_name')} placeholder="Workflow default" /></div><div><label className={labelClass}>From email <span className="font-normal">(optional)</span></label><input type="email" className={inputClass} value={String(config.sender_email || '')} onChange={(event) => update({ ...config, sender_email: event.target.value }, 'sender_email')} placeholder="Enabled Email Account" /></div></div>
...
function EventContextNotice({ topic, events, primaryDoctype, usage, sourceMode = 'enrolled_record' }: { topic: string; events: BusinessEventType[]; primaryDoctype?: string; usage: 'trigger' | 'wait'; sourceMode?: 'enrolled_record' | 'action_output' }) {
	const definition = events.find((event) => event.topic === topic)
	if (!definition) return null
	const connected = definition.producer_status === 'native'
	return <div className={`mt-2 rounded-lg border px-2.5 py-2 text-[9.5px] leading-4 ${connected ? 'border-emerald-200 bg-emerald-50/80 text-emerald-900 dark:border-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-200' : 'border-amber-200 bg-amber-50/80 text-amber-900 dark:border-amber-800 dark:bg-amber-500/10 dark:text-amber-200'}`}>
		<strong className="block">{sourceMode === 'action_output' ? 'How the event is matched' : `How this works for ${primaryDoctype || 'this record'}`}</strong>
		<span className="mt-0.5 block">{sourceMode === 'action_output' ? 'Only an event identifying the exact record or message from the selected earlier action can release this wait.' : definition.record_resolution}</span>
...
function EventEnrollmentEditor({ config, typeVersion, events, recordFields, primaryDoctype, update }: { config: NodeConfig; typeVersion: number; events: BusinessEventType[]; recordFields: FieldCatalogItem[]; primaryDoctype?: string; update(config: NodeConfig, key: string): void }) {
	...
	return <>
		<InspectorSection title={`When an event happens to this ${primaryDoctype || 'record'}`} description={`Only events that can resolve to the enrolled ${primaryDoctype || 'record'} are shown. Event groups are OR: any one can enroll the record.`}>
			<div className="space-y-3" data-config-path={legacy ? 'event_topic' : 'events'}>{entries.map((entry, index) => <section className="rounded-xl border border-[var(--border-color)] bg-[var(--subtle-fg)] p-3" key={entry.id}><div className="mb-2 flex items-center justify-between"><strong className="text-heading text-[10.5px]">Event {index + 1}</strong>{!legacy && entries.length > 1 && <button type="button" className="icon-button !size-7 hover:!text-red-600" onClick={() => setEntries(entries.filter((_, entryIndex) => entryIndex !== index), `events:${index}:remove`)} aria-label={`Remove event ${index + 1}`}><Trash2 size={12} /></button>}</div><label className={labelClass}>Event</label><EventTopicPicker value={entry.event_topic} events={events} onChange={(event_topic) => setEntries(entries.map((item, entryIndex) => entryIndex === index ? { ...item, event_topic, event_filter: null } : item), `events:${index}:topic`)} /><EventContextNotice topic={entry.event_topic} events={events} primaryDoctype={primaryDoctype} usage="trigger" /><div className="mt-3"><EventCriteriaEditor topic={entry.event_topic} value={entry.event_filter} events={events} onChange={(event_filter) => setEntries(entries.map((item, entryIndex) => entryIndex === index ? { ...item, event_filter } : item), `events:${index}:filter`)} /></div></section>)}</div>
			{!legacy && <button type="button" className="btn-core btn-secondary w-full !text-[10px]" disabled={entries.length >= 20} onClick={() => setEntries([...entries, { id: `event-${crypto.randomUUID()}`, event_topic: '', event_filter: null }], 'events:add')}><Plus size={12} />Add another OR event ({entries.length}/20)</button>}
			<Hint>The integration must signal the selected event with this enrolled record. Re-enrollment policy decides whether later matching occurrences create another run.</Hint>
		</InspectorSection>
...
function MultiTriggerEditor({ config, events, fields, primaryDoctype, update }: { config: NodeConfig; events: BusinessEventType[]; fields: FieldCatalogItem[]; primaryDoctype?: string; update(config: NodeConfig, key: string): void }) {
	...
	return <InspectorSection title="Enroll when any trigger matches" description={`Each row is an OR enrollment path for the same ${primaryDoctype || 'record'} workflow. Created, changed, criteria, and business events can be mixed safely.`}><div className="space-y-3" data-config-path="triggers">{entries.map((entry, index) => { const entryConfig = entry.config || {}; return <section className="rounded-xl border border-[var(--border-color)] bg-[var(--subtle-fg)] p-3" key={entry.id}><div className="mb-3 flex items-center justify-between"><strong className="text-heading text-[10.5px]">Trigger {index + 1}</strong><button type="button" className="icon-button !size-7 hover:!text-red-600" disabled={entries.length <= 2} onClick={() => setEntries(entries.filter((_, entryIndex) => entryIndex !== index), `triggers:${index}:remove`)} aria-label={`Remove trigger ${index + 1}`}><Trash2 size={12} /></button></div><label className={labelClass}>Enrollment moment</label><select className={inputClass} value={entry.type} onChange={(event) => replace(index, { type: event.target.value, config: defaultConfig(event.target.value) })}><option value="trigger.document_insert">Record is created</option><option value="trigger.document_change">Record changes</option><option value="trigger.filter_criteria">Record meets criteria</option><option value="trigger.event">Business event occurs</option></select>{entry.type === 'trigger.document_change' && <div className="mt-3"><label className={labelClass}>Only when these fields change <span className="text-muted font-normal">(optional)</span></label><MultiValueInput values={(Array.isArray(entryConfig.watch_fields) ? entryConfig.watch_fields : []).map(String)} onChange={(watch_fields) => replace(index, { config: { ...entryConfig, watch_fields } })} loadOptions={async (search) => fields.filter((field) => !search || `${field.label} ${field.fieldname}`.toLowerCase().includes(search.toLowerCase())).map((field) => ({ value: field.fieldname, label: field.label, description: field.fieldtype }))} placeholder="Any field change" ariaLabel={`Trigger ${index + 1} watched fields`} /></div>}{entry.type === 'trigger.event' && <div className="mt-3"><label className={labelClass}>Business event</label><EventTopicPicker value={String(entryConfig.event_topic || '')} events={events} onChange={(event_topic) => replace(index, { config: { ...entryConfig, event_topic, event_filter: null } })} /><EventContextNotice topic={String(entryConfig.event_topic || '')} events={events} primaryDoctype={primaryDoctype} usage="trigger" /><div className="mt-3"><EventCriteriaEditor topic={String(entryConfig.event_topic || '')} value={entryConfig.event_filter} events={events} onChange={(event_filter) => replace(index, { config: { ...entryConfig, event_filter } })} /></div></div>}{entry.type !== 'trigger.event' && <div className="mt-3"><div className="mb-2 flex items-center justify-between"><label className={labelClass}>Record criteria {entry.type === 'trigger.filter_criteria' ? '' : '(optional)'}</label>{Boolean(entryConfig.condition) && <button type="button" className="btn-core btn-ghost !min-h-7 !px-2 !text-[9px]" onClick={() => replace(index, { config: { ...entryConfig, condition: null } })}>Clear</button>}</div>{entryConfig.condition ? <ConditionExpressionEditor expression={parseCondition(entryConfig.condition)} fields={fields} primaryDoctype={primaryDoctype} depth={0} onChange={(condition) => replace(index, { config: { ...entryConfig, condition } })} /> : <button type="button" className="btn-core btn-secondary w-full !text-[10px]" onClick={() => replace(index, { config: { ...entryConfig, condition: emptyPredicate() } })}><Plus size={12} />Add criteria</button>}</div>}{entry.type === 'trigger.event' && <div className="mt-3"><label className={labelClass}>Additional record criteria <span className="text-muted font-normal">(optional)</span></label>{entryConfig.condition ? <ConditionExpressionEditor expression={parseCondition(entryConfig.condition)} fields={fields} primaryDoctype={primaryDoctype} depth={0} onChange={(condition) => replace(index, { config: { ...entryConfig, condition } })} /> : <button type="button" className="btn-core btn-secondary w-full !text-[10px]" onClick={() => replace(index, { config: { ...entryConfig, condition: emptyPredicate() } })}><Plus size={12} />Add record criteria</button>}</div>}</section> })}</div><button type="button" className="btn-core btn-secondary w-full !text-[10px]" disabled={entries.length >= 20} onClick={() => setEntries([...entries, { id: `trigger-${crypto.randomUUID()}`, type: 'trigger.filter_criteria', config: { condition: null } }], 'triggers:add')}><Plus size={12} />Add another OR trigger ({entries.length}/20)</button><Hint>One matching subscription creates one idempotent enrollment occurrence. Re-enrollment settings still control repeat runs.</Hint></InspectorSection>
}
...
export function EventWaitEditor({ config, typeVersion, events, outputNodes, update, nodeId, primaryDoctype, timeoutPathConnected = false }: { config: NodeConfig; typeVersion: number; events: BusinessEventType[]; outputNodes: WorkflowNode[]; update(config: NodeConfig, key: string): void; nodeId: string; primaryDoctype?: string; timeoutPathConnected?: boolean }) {
	...
	return <InspectorSection title="Wait until an event occurs" description="The event must happen while this record is waiting. Earlier activity does not release the delay.">
		...
		<div data-config-path="event_topic"><label className={labelClass}>Event</label><EventTopicPicker value={topic} events={visibleEvents} onChange={chooseEvent} /><EventContextNotice topic={topic} events={events} primaryDoctype={primaryDoctype} usage="wait" sourceMode={dataSource} /></div>
		...
		<div data-config-path="timeout_mode"><label className={labelClass}>Maximum wait</label><div className="grid grid-cols-2 gap-2"><button type="button" className={`btn-core ${timeoutMode === 'duration' ? 'btn-primary' : 'btn-secondary'} !text-[10px]`} onClick={() => update({ ...config, timeout_mode: 'duration' }, 'timeout_mode')}>For a set time</button><button type="button" className={`btn-core ${timeoutMode === 'indefinite' ? 'btn-primary' : 'btn-secondary'} !text-[10px]`} disabled={timeoutPathConnected || typeVersion < 2} onClick={() => update({ ...config, timeout_mode: 'indefinite', branch_on_timeout: 0 }, 'timeout_mode')}>As long as possible</button></div></div>
		...
		{typeVersion >= 2 && timeoutMode === 'duration' && <label className="text-body flex items-start gap-2 text-[11px]"><input type="checkbox" className="mt-0.5" checked={Boolean(config.branch_on_timeout)} disabled={Boolean(config.branch_on_timeout) && timeoutPathConnected} onChange={(event) => update({ ...config, branch_on_timeout: event.target.checked ? 1 : 0 }, 'branch_on_timeout')} /><span><strong className="text-heading block">Create a separate timeout path</strong><span className="text-muted block text-[9.5px] leading-4">Leave off when both outcomes should continue to the same next action.</span></span></label>}
		{timeoutPathConnected && <Hint>The timeout path is connected. Remove or move that path before switching to one path or waiting indefinitely.</Hint>}
...
function BranchFilterGroupsEditor({ value, fields, primaryDoctype, onChange }: { value: ConditionExpression | null; fields: FieldCatalogItem[]; primaryDoctype?: string; onChange(condition: ConditionExpression): void }) {
	const groups = conditionToFilterGroups(value)
	if (!groups) return <div className="space-y-3 rounded-xl border border-amber-200 bg-amber-50/70 p-3 dark:border-amber-800 dark:bg-amber-500/10"><div><strong className="text-amber-800 text-[10.5px] dark:text-amber-200">Advanced criteria from an older draft</strong><p className="mt-1 text-[9.5px] leading-4 text-amber-700 dark:text-amber-300">This path still runs unchanged. New paths use simple AND groups separated by OR.</p></div><ConditionExpressionEditor expression={parseCondition(value)} fields={fields} primaryDoctype={primaryDoctype} depth={0} onChange={onChange} /><button type="button" className="btn-core btn-secondary w-full !text-[10px]" onClick={() => onChange(filterGroupsToCondition([[emptyPredicate()]]))}>Replace with simple filter groups</button></div>
	const commit = (next: ConditionPredicate[][]) => onChange(filterGroupsToCondition(next))
	return <div className="space-y-2.5">{groups.map((group, groupIndex) => <div key={groupIndex}>{groupIndex > 0 && <div className="relative my-3 flex items-center justify-center"><span className="absolute inset-x-0 h-px bg-[var(--border-color)]" /><strong className="relative rounded-full border border-brand-200 bg-white px-3 py-1 text-[9px] text-brand-700 dark:bg-[#18212b] dark:text-brand-200">OR</strong></div>}<section className="rounded-xl border border-[var(--border-color)] bg-[var(--subtle-fg)] p-3"><div className="mb-2.5 flex items-start justify-between gap-3"><div><strong className="text-heading block text-[10.5px]">Filter group {groupIndex + 1}</strong><span className="text-muted mt-0.5 block text-[9px]">Every condition below must match (AND)</span></div>{groups.length > 1 && <button type="button" className="icon-button !size-7 hover:!text-red-600" onClick={() => commit(groups.filter((_, index) => index !== groupIndex))} aria-label={`Remove filter group ${groupIndex + 1}`}><Trash2 size={12} /></button>}</div><div className="space-y-2">{group.map((predicate, predicateIndex) => <div className="rounded-lg border border-[var(--border-color)] bg-white/60 p-2.5 dark:bg-white/5" key={predicateIndex}><div className="mb-2 flex items-center justify-between"><span className="text-light text-[8.5px] font-bold uppercase">Condition {predicateIndex + 1}</span>{group.length > 1 && <button type="button" className="icon-button !size-6 hover:!text-red-600" onClick={() => commit(groups.map((item, index) => index === groupIndex ? item.filter((_, conditionIndex) => conditionIndex !== predicateIndex) : item))} aria-label={`Remove condition ${predicateIndex + 1}`}><Trash2 size={11} /></button>}</div><ConditionExpressionEditor expression={predicate} fields={fields} primaryDoctype={primaryDoctype} depth={1} onChange={(condition) => { if (condition.kind !== 'predicate') return; commit(groups.map((item, index) => index === groupIndex ? item.map((candidate, conditionIndex) => conditionIndex === predicateIndex ? condition : candidate) : item)) }} /></div>)}</div><button type="button" className="btn-core btn-secondary mt-2.5 w-full !text-[10px]" onClick={() => commit(groups.map((item, index) => index === groupIndex ? [...item, emptyPredicate()] : item))}><Plus size={12} />Add AND condition</button></section></div>)}<button type="button" className="btn-core btn-secondary w-full !text-[10px]" onClick={() => commit([...groups, [emptyPredicate()]])}><Plus size={12} />Add OR filter group</button></div>
}

function CriteriaBranchesEditor({ config, fields, primaryDoctype, update, removeBranch }: { config: NodeConfig; fields: FieldCatalogItem[]; primaryDoctype?: string; update(config: NodeConfig, key: string): void; removeBranch(handle: string, branches: CriteriaBranch[]): void }) {
	...
	const conditionCount = (condition: ConditionExpression | null): number => !condition ? 0 : condition.kind === 'predicate' ? (condition.field ? 1 : 0) : condition.children.reduce((total, child) => total + conditionCount(child), 0)
	...
	return <InspectorSection title="Choose the path for each record" description="Paths are checked from top to bottom. The first matching path is used; everyone else follows None.">
		<div className="rounded-xl border border-blue-200 bg-blue-50/70 p-3 text-[10px] leading-4 text-blue-800 dark:border-blue-900 dark:bg-blue-500/10 dark:text-blue-200">
			<strong className="block">How routing works</strong>
			<span className="mt-1 block">Conditions in one filter group use AND. Add another filter group for OR. Put the most specific path first; no manual else condition is needed.</span>
		</div>
		...
		<div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-3 text-[10.5px] text-emerald-800 dark:border-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-300"><strong className="block">None — everyone else</strong><span className="mt-1 block">Automatic final path when none of the paths above match. It cannot be removed.</span></div>
	</InspectorSection>
}
        ...
        <div className="flex w-full min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
			...
			<div className="flex shrink-0 gap-1">
			  {node.id !== graph?.start_node_id && <><button className="icon-button" onClick={() => actions.copyNode(node.id)} aria-label="Copy node" title="Copy this step (Ctrl/Cmd+C)"><Copy size={14} /></button><button className="icon-button" onClick={() => actions.duplicateNode(node.id)} aria-label="Duplicate node" title="Duplicate this step below"><Plus size={14} /></button><button className="icon-button" onClick={() => actions.copySection(node.id)} aria-label="Copy connected path" title="Copy this step and its exclusively connected downstream path"><Network size={14} /></button><button className="icon-button" onClick={() => actions.duplicateSection(node.id)} aria-label="Duplicate connected path" title="Duplicate this connected path below"><Network size={14} /><Plus className="-ml-1 -mt-2" size={9} /></button><button className="icon-button hover:!border-red-200 hover:!bg-red-50 hover:!text-red-600 dark:hover:!bg-red-500/10" onClick={() => actions.removeNode(node.id)} aria-label="Delete node"><Trash2 size={16} /></button></>}
			</div>
        </div>
      </div>
	  <div className="flex shrink-0 items-center justify-between border-b border-[var(--border-color)] bg-[var(--subtle-fg)] px-4 py-2 max-lg:hidden">
		<span className="text-light text-[9px] font-bold uppercase tracking-[0.12em]">Panel width · {width}px</span>
		<button type="button" className="btn-core btn-secondary !min-h-7 !px-2.5 !text-[9.5px]" disabled={!canToggleWidth} onClick={onToggleExpanded} title={!canToggleWidth ? 'This is the widest size that leaves room for the canvas' : undefined}>{expanded ? <PanelRightClose size={12} /> : <PanelRightOpen size={12} />}{expanded ? 'Use standard width' : canToggleWidth ? 'Make panel wider' : 'Maximum width'}</button>
	  </div>

	  <div className="inspector-scroll-region min-h-0 min-w-0 flex-1 space-y-3 overflow-y-scroll overflow-x-hidden p-4 pb-12" onWheel={(event) => event.stopPropagation()}>
		...
		{node.type === 'condition.if_else' && node.type_version >= 2 && <CriteriaBranchesEditor config={config} fields={conditionFields} update={update} primaryDoctype={primaryDoctype} removeBranch={(handle, branches) => actions.updateNodeAndRemoveEdges(node.id, { ...config, branches }, (graph?.edges || []).filter((edge) => edge.source === node.id && edge.source_handle === handle).map((edge) => edge.id), `node:${node.id}:branches:remove`)} />}
		...
		{node.type === 'delay.fixed' && <InspectorSection title="Delay settings" description="Choose a readable unit; the run is stored durably while it waits."><DurationEditor key={node.id} seconds={Number(config.seconds || 3600)} onChange={(seconds, duration, unit) => update({ ...config, seconds, duration, duration_unit: unit }, 'duration')} /><Hint>Use minutes, hours, days, or weeks instead of converting long waits to seconds. Maximum duration is one year.</Hint></InspectorSection>}
		...
		{node.type === 'trigger.any' && <MultiTriggerEditor config={config} events={triggerEventTypes} fields={conditionFields} primaryDoctype={primaryDoctype} update={update} />}

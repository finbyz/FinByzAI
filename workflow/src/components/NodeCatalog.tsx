...
export function NodeCatalog() {
  ...
  const grouped = useMemo(() => {
    const visible = items.filter((item) => !item.type.startsWith('trigger.') && `${item.label} ${item.description}`.toLowerCase().includes(query.toLowerCase()))
    return visible.reduce<Record<string, NodeCatalogItem[]>>((groups, item) => {
      groups[item.category] = [...(groups[item.category] || []), item]
      return groups
    }, {})
  }, [items, query])
  const triggerItems = items.filter((item) => item.type.startsWith('trigger.'))

  return (
    <>
      {mobileOpen && <button type="button" className="absolute inset-0 z-[35] hidden bg-slate-950/25 backdrop-blur-[1px] max-lg:block" onClick={() => setMobileOpen(false)} aria-label="Close step catalog" />}
      <aside aria-label="Workflow step catalog" className={`editor-side-panel flex flex-col h-full overflow-hidden border-r border-[var(--border-color)] bg-white/70 dark:bg-[#18212b]/80 backdrop-blur-2xl transition-[transform,visibility] max-lg:absolute max-lg:inset-y-0 max-lg:left-0 max-lg:z-40 max-lg:w-full sm:max-lg:w-80 max-lg:shadow-2xl ${mobileOpen ? 'max-lg:visible max-lg:translate-x-0' : 'max-lg:invisible max-lg:-translate-x-full'}`}>
        <div className="shrink-0 sticky top-0 z-10 border-b border-[var(--border-color)] bg-white/50 dark:bg-[#18212b]/50 px-4 pb-4 pt-4 backdrop-blur-md">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-heading text-[13px] font-bold">Add a step</p>
              <p className="text-muted mt-0.5 text-[11px]">Build the next part of your workflow</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="magic-orb !size-8 !rounded-[9px]"><Sparkles size={14} /></span>
              <button className="icon-button lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close catalog"><X size={16} /></button>
            </div>
          </div>
                ...
                {rows.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className="catalog-item group flex w-full items-start gap-3 px-2.5 py-2.5 text-left"
                    >

...
describe('EventWaitEditor', () => {
	...
	it('supports indefinite waits but protects an already-connected timeout path', () => {
		const update = vi.fn()
		...
		expect(screen.getByRole('button', { name: 'As long as possible' })).toBeDisabled()
		expect(screen.getByText(/timeout path is connected/i)).toBeInTheDocument()
	})
})

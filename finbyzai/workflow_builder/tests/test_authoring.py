...
from finbyzai.workflow_builder import engine
...
class TestAutomationAuthoring(IntegrationTestCase):
	...
	def test_event_wait_releases_to_event_branch_and_timeout_branch(self):
		def create_run(key: str):
			...
			graph = created["graph"]
			graph["nodes"].extend([
				{"id": "wait", "type": "delay.until_event", "type_version": 1, "config": {"event_topic": "crm.lead.qualified", "timeout_seconds": 3600}},
		...
		with patch.object(engine, "_queue_token"):
			self.assertEqual(release_event_waiters("crm.lead.qualified", {"score": 10}), 1)

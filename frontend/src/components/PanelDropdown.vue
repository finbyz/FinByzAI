...
// Inline dropdown (rendered within #copilot-root, never teleported) so its styles
// stay scoped to the panel.
const props = defineProps({
	modelValue: { default: null },
	items: { type: Array, default: () => [] }, // [{ value, label, logo?, hint?, group? }]
...
// horizontally if either edge would spill past #copilot-root. Measured once per
...
watch(open, async (v) => {
	...
	await nextTick();
	const panel = document.getElementById("copilot-root");
		...
		<template v-if="open">
			...
			>
				...
				<div class="max-h-80 overflow-y-auto p-1.5 space-y-0.5">
					<template v-for="(item, i) in filtered" :key="item.value ?? `__d${i}`">
						<!-- One heading per provider, so 76 models read as a short list of vendors. -->
						<div
							v-if="item.group && item.group !== filtered[i - 1]?.group"
							class="px-3 pb-1 pt-2 text-2xs font-semibold uppercase tracking-wide text-ink-gray-4"
						>
							{{ item.group }}
						</div>
						<button
						...
						>
							<img
								v-if="item.logo"
								:src="item.logo"
								class="copilot-logo h-3.5 w-3.5 shrink-0"
								alt=""
							/>
